...
last_time = time.time()
frame_skip = 2  # Skip every 10 frames
while ret:
    frame_nmr += 1
    ret, frame = cap.read()
        ...
        for license_plate in license_plates.boxes.data.tolist():
            ...
            if car_id != -1:
                ...
                if license_plate_text is not None and license_plate_text not in seen_plates and plate_height >= height_threshold:
                    ...
                    open_gate_text = "Open gate"
                elif license_plate_text is not None and license_plate_text in seen_plates:
                    open_gate_text = "Open gate"
                else:
                    open_gate_text = "Close gate"
                ...
                # Draw the text
                if blur_height >= height_threshold:
                    box_color = (0, 255, 0)
                else:
                    box_color = (0, 0, 255)
                
                cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), box_color, 2) # color of the rectangle around the plate
                cv2.putText(frame, number_plate_text, text_position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, box_color, 2) # color of the text
                cv2.rectangle(frame, (int(xcar1), int(ycar1)), (int(xcar2), int(ycar2)), box_color, 2) # color of the rectangle around the car
                cv2.imshow('frame', frame)
